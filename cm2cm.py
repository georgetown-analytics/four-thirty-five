# This script downloads all the the (zipped) FEC detailed Committee to Committee payment Master data
...
YEARS = list(range(1996, 2016, 2)) # 2016 is excluded  - changed for detailed committee to committee master file
HEADERS_URL = 'http://classic.fec.gov/finance/disclosure/metadata/oth_header_file.csv'  #changed from http://www.fec.gov/finance/disclosure/metadata/webl_header_file.csv'
...
oname = "./oth-%s-%s.csv" % (YEARS[0], YEARS[-1])
...
def url_for_year(year):
    """e.g. year=2016, result: 'ftp://ftp.fec.gov/FEC/2016/oth16.zip'"""
    y = str(year)
    return 'ftp://ftp.fec.gov/FEC/%s/oth%s.zip' % (y, y[2:])
