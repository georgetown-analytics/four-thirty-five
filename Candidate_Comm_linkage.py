...
YEARS = list(range(2000, 2016, 2)) # 2018 is excluded  - changed for detailed candidate committee linkage master file
HEADERS_URL = 'http://classic.fec.gov/finance/disclosure/metadata/ccl_header_file.csv'  #changed from http://www.fec.gov/finance/disclosure/metadata/webl_header_file.csv'
...
oname = "./ccl-%s-%s.csv" % (YEARS[0], YEARS[-1])
...
def url_for_year(year):
    """e.g. year=2016, result: 'ftp://ftp.fec.gov/FEC/2016/ccl16.zip'"""
    y = str(year)
    return 'ftp://ftp.fec.gov/FEC/%s/ccl%s.zip' % (y, y[2:])
