# This script downloads all the the (zipped) FEC detailed Operating Expense data
...
YEARS = list(range(2004, 2018, 2)) # 2018 is excluded  - changed for detailed operating expense  master file
HEADERS_URL = 'http://classic.fec.gov/finance/disclosure/metadata/oppexp_header_file.csv'  #changed from http://www.fec.gov/finance/disclosure/metadata/webl_header_file.csv'
...
oname = "./opex-%s-%s.csv" % (YEARS[0], YEARS[-1])
...
def url_for_year(year):
    """e.g. year=2016, result: 'ftp://ftp.fec.gov/FEC/2016/oppexp16.zip'"""
    y = str(year)
    return 'ftp://ftp.fec.gov/FEC/%s/oppexp%s.zip' % (y, y[2:])
