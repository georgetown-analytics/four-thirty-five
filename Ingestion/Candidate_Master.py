# This script downloads all the the (zipped) FEC detailed Candidate Master data
# for House and Senate races, as found at the FEC:
# http://classic.fec.gov/finance/disclosure/ftpdet.shtml
#
# The result is a CSV file of the compiled records, from 2004 - 2016
...
YEARS = list(range(2004, 2016, 2)) # 2018 is excluded  - changed for detailed candidate master file
HEADERS_URL = 'http://classic.fec.gov/finance/disclosure/metadata/cn_header_file.csv'  #changed from http://www.fec.gov/finance/disclosure/metadata/webl_header_file.csv'
...
oname = "./can-%s-%s.csv" % (YEARS[0], YEARS[-1])
...
def url_for_year(year):
    """e.g. year=2014, result: 'ftp://ftp.fec.gov/FEC/2016/cn16.zip'"""
    y = str(year)
    return 'ftp://ftp.fec.gov/FEC/%s/cn%s.zip' % (y, y[2:])
